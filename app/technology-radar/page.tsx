                    ...
                    <Ring key={r.r} quadrant={q.q} ring={r.r} label={r.label}>
                      ...
                      {blips
                        ...
                        .map(i => {
                          blip = blip + 1;
                          return (
                            <Blip key={i._id} blip={blip} label={i.technology || i.technique || 'empty'} quadrant={q.q} />
                          )
                        })}
...
function Quadrant(props: IQuadrant) {
  ...
  return (
    <div className={clsx('flex border-2 border-neutral-200 dark:border-neutral-800 rounded-md overflow-hidden', {
      'flex-col-reverse': quadrant.includes('top'),
      'flex-col': quadrant.includes('bottom'),
    })}>
      {children}
      <div className={clsx('flex-none text-2xl font-bold px-4 py-4', {
...
function Ring(props: IRing) {
  ...
  return (
    <div className={clsx('flex-none h-14', {
      ...
      <div className={clsx('p-2 flex', {
        'flex-row-reverse': quadrant.includes('left'),
        'flex-row': quadrant.includes('right'),
      })}>
        {children}
        <div className={clsx('flex-grow text-sm pt-2 px-2', {
          'text-left': quadrant.includes('left'),
          'text-right': quadrant.includes('right'),
        })}>{label}</div>
      </div>
